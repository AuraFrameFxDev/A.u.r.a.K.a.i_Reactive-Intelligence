...
class KaiAgent
    ...
    constructor(
                ...
                val securityResponse =
                    when (securityAssessment.riskLevel) {
                        ...
                        ThreatLevel.CRITICAL ->
                            generateCriticalSecurityResponse(
                                interaction,
                                securityAssessment,
                            )
                    }
        ...
        private suspend fun handleSecurityAnalysis(request: AgentRequest): Map<String, Any> {
            val target =
                request.context?.get("target") as? String
        ...
        private suspend fun handleThreatAssessment(request: AgentRequest): Map<String, Any> {
            val threatData =
                request.context?.get("threat_data") as? String
        ...
        private suspend fun handleCodeReview(request: AgentRequest): Map<String, Any> {
            val code =
                request.context?.get("code") as? String
        ...
        private fun calculateAnalysisConfidence(
            indicators: List<String>,
            threatLevel: ThreatLevel,
        ): Float = minOf(0.95f, 0.6f + (indicators.size * 0.1f))
