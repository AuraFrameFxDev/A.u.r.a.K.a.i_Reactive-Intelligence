...
data class AgentHierarchy(
    ...
    companion object {
        ...
        fun getAgentConfig(name: String): HierarchyAgentConfig? = (MASTER_AGENTS + auxiliaryAgents).find { it.name == name }

        fun getAgentsByPriority(): List<HierarchyAgentConfig> = (MASTER_AGENTS + auxiliaryAgents).sortedBy { it.priority }
    }
}
