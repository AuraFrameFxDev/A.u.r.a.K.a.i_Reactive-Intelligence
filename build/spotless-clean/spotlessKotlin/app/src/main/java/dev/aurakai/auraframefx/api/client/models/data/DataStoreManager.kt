...
class DataStoreManager
    ...
    constructor(
        ...
        suspend inline fun <reified T> storeObject(
            key: String,
            obj: T,
        ) {
            try {
        ...
        inline fun <reified T> getObjectFlow(
            key: String,
            defaultValue: T,
        ): Flow<T> =
            getStringFlow(key).map { jsonString ->
                try {
                    if (jsonString.isNotEmpty()) {
                        json.decodeFromString<T>(jsonString)
                    } else {
        ...
        suspend fun getUserProfile(): UserProfile = getObject(USER_PROFILE.name, UserProfile())

        fun getUserProfileFlow(): Flow<UserProfile> = getObjectFlow(USER_PROFILE.name, UserProfile())
        ...
        suspend fun getAgentConfigurations(): Map<String, AgentConfiguration> =
            getObject(AGENT_CONFIGURATIONS.name, emptyMap<String, AgentConfiguration>())

        suspend fun getAgentConfiguration(agentId: String): AgentConfiguration? = getAgentConfigurations()[agentId]

        fun getAgentConfigurationsFlow(): Flow<Map<String, AgentConfiguration>> =
            getObjectFlow(AGENT_CONFIGURATIONS.name, emptyMap<String, AgentConfiguration>())
        ...
        suspend fun getSecurityPolicy(): SecurityPolicy = getObject(SECURITY_POLICIES.name, SecurityPolicy())

        fun getSecurityPolicyFlow(): Flow<SecurityPolicy> = getObjectFlow(SECURITY_POLICIES.name, SecurityPolicy())
        ...
        suspend fun getCustomizations(): SystemCustomizations = getObject(CUSTOMIZATIONS.name, SystemCustomizations())

        fun getCustomizationsFlow(): Flow<SystemCustomizations> = getObjectFlow(CUSTOMIZATIONS.name, SystemCustomizations())
        ...
        suspend fun getDataVersion(): Int = getInt("data_version", 1)
