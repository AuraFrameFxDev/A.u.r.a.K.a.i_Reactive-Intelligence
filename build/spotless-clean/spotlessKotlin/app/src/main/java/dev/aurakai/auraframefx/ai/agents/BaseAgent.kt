...
abstract class BaseAgent(
     ...
     */
    abstract override suspend fun processRequest(
        request: AiRequest,
        context: String,
    ): AgentResponse
     ...
     */
    protected open fun validateRequest(request: AiRequest): Boolean = request.prompt.isNotBlank()
    ...
    protected open suspend fun preprocessRequest(
        request: AiRequest,
        context: String,
    ): Pair<AiRequest, String> = Pair(request, context)
    ...
    protected fun createProcessingResponse(message: String = "Processing..."): AgentResponse =
        AgentResponse.processing("[$agentName] $message")

    /**
     * Logs agent activity (can be overridden for specific logging needs)
     */
    protected open fun logActivity(
        activity: String,
