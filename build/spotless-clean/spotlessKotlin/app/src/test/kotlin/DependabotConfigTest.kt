...
private fun validateDependabotConfig(cfg: DependabotConfig): List<String> {
    ...
    cfg.updates.forEachIndexed { idx, u ->
        ...
        if (u.scheduleInterval.isNullOrBlank()) {
            errors.add(
                "$at.schedule.interval must be one of ${allowedIntervals.joinToString(
                    ", ",
                )}, found: ${u.scheduleInterval?.let { "'$it'" } ?: "null"}",
            )
 ...
 */
private fun parseAndValidate(yaml: String): List<String> = validateDependabotConfig(parseDependabotConfig(yaml))
...
class DependabotConfigTest {
    ...
    fun invalidPackageEcosystem_reportsError_basedOnPRSnippet() {
        ...
        assertFalse(
            errors.any {
                it.contains("schedule.interval must be") &&
                    it.contains(
                        "found: 'weekly'",
                    )
            },
    ...
    fun missingScheduleInterval_reportsError() {
        ...
        assertTrue(
            errors.any {
                it.contains("schedule.interval must be one of") &&
                    it.contains(
                        "found: null",
                    )
            },
    ...
    fun multipleUpdates_aggregatesErrors_andIdentifiesIndices() {
        ...
        assertTrue(
            errors.any {
                it.contains("updates[1].schedule.interval") &&
                    it.contains(
                        "found: 'yearly'",
                    )
            },
