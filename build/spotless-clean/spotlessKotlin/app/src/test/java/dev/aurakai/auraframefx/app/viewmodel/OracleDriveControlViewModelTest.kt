...
class OracleDriveControlViewModelTest {
    ...
    @Test
    fun `startDrive should update state to driving when successful`() =
        runTest {
    ...
    @Test
    fun `startDrive should handle network error gracefully`() =
        runTest {
    ...
    @Test
    fun `stopDrive should update state to idle when successful`() =
        runTest {
            coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
            viewModel.startDrive()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `stopDrive should handle repository error`() =
        runTest {
    ...
    @Test
    fun `pauseDrive should update state to paused when successful`() =
        runTest {
            coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
            viewModel.startDrive()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `resumeDrive should update state to driving from paused`() =
        runTest {
            coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
            viewModel.startDrive()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `updateSpeed should call repository with correct value`() =
        runTest {
            val speed = 50.0
    ...
    @Test
    fun `updateSpeed should handle negative values gracefully`() =
        runTest {
    ...
    @Test
    fun `updateSpeed should handle excessive values gracefully`() =
        runTest {
    ...
    @Test
    fun `changeDirection should update direction when valid`() =
        runTest {
    ...
    @Test
    fun `changeDirection should handle repository error`() =
        runTest {
            val direction = Direction.REVERSE
            val errorMessage = "Failed to change direction"
            coEvery { mockRepository.changeDirection(direction) } returns
                Result.failure(
                    Exception(
                        errorMessage,
                    ),
    ...
    @Test
    fun `emergency stop should immediately stop drive and update state`() =
        runTest {
            coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
            viewModel.startDrive()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `emergency stop should handle repository failure`() =
        runTest {
    ...
    @Test
    fun `reset should clear error state and return to idle`() =
        runTest {
            coEvery { mockRepository.emergencyStop() } returns Result.success(Unit)
            viewModel.emergencyStop()
            testDispatcher.scheduler.advanceUntilIdle()
    ...
    @Test
    fun `concurrent operations should be handled correctly`() =
        runTest {
            coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
            coEvery { mockRepository.stopDrive() } returns Result.success(Unit)
    ...
    @Test
    fun `multiple speed updates should debounce correctly`() =
        runTest {
    ...
    @Test
    fun `loading state should be managed correctly across operations`() =
        runTest {
    ...
    @Test
    fun `view model should handle null repository responses gracefully`() =
        runTest {
    ...
    @Test
    fun `view model should validate input parameters`() =
        runTest {
...
@Test
fun `updateSpeed should accept zero speed value`() =
    runTest {
        val zeroSpeed = 0.0
...
@Test
fun `updateSpeed should handle infinity values`() =
    runTest {
...
@Test
fun `updateSpeed should handle negative infinity values`() =
    runTest {
...
@Test
fun `updateSpeed should handle very small positive values`() =
    runTest {
...
@Test
fun `changeDirection should not be allowed during emergency stop state`() =
    runTest {
...
@Test
fun `startDrive should not be allowed during emergency stop state`() =
    runTest {
        // First trigger emergency stop
        coEvery { mockRepository.emergencyStop() } returns Result.success(Unit)
        viewModel.emergencyStop()
...
@Test
fun `pauseDrive should not be allowed when not driving`() =
    runTest {
...
@Test
fun `resumeDrive should not be allowed when not paused`() =
    runTest {
...
@Test
fun `updateSpeed should handle repository timeout gracefully`() =
    runTest {
        val speed = 25.0
        coEvery { mockRepository.updateSpeed(speed) } returns
            Result.failure(
...
@Test
fun `startDrive should handle IOException gracefully`() =
    runTest {
...
@Test
fun `viewModel should handle multiple rapid state changes`() =
    runTest {
        coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
        coEvery { mockRepository.pauseDrive() } returns Result.success(Unit)
        coEvery { mockRepository.resumeDrive() } returns Result.success(Unit)
...
@Test
fun `multiple emergency stops should be handled gracefully`() =
    runTest {
        coEvery { mockRepository.emergencyStop() } returns Result.success(Unit)
...
@Test
fun `network state changes should not affect ongoing operations`() =
    runTest {
        coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
        every { mockNetworkManager.isConnected() } returns true
...
@Test
fun `speed updates should be validated against upper boundary`() =
    runTest {
        val maxSpeed = 999.9
...
@Test
fun `direction changes should be logged for audit purposes`() =
    runTest {
...
@Test
fun `error messages should be cleared after successful operations`() =
    runTest {
...
@Test
fun `repository operations should handle concurrent access gracefully`() =
    runTest {
        coEvery { mockRepository.updateSpeed(any()) } returns Result.success(Unit)
...
@Test
fun `loading state should remain consistent during overlapping operations`() =
    runTest {
        coEvery { mockRepository.startDrive(any()) } coAnswers {
...
@Test
fun `observer removal should not cause memory leaks`() =
    runTest {
...
@Test
fun `emergency stop should override all other pending operations`() =
    runTest {
        coEvery { mockRepository.startDrive(any()) } coAnswers {
...
@Test
fun `reset from error state should clear all error conditions`() =
    runTest {
...
@Test
fun `speed validation should handle floating point precision issues`() =
    runTest {
...
@Test
fun `viewModel should handle repository returning unexpected result types`() =
    runTest {
...
@Test
fun `consecutive speed updates within threshold should be debounced`() =
    runTest {
        coEvery { mockRepository.updateSpeed(any()) } returns Result.success(Unit)
...
@Test
fun `drive operations should validate network connectivity when required`() =
    runTest {
        every { mockNetworkManager.isConnected() } returns false
...
@Test
fun `error recovery should allow normal operations after reset`() =
    runTest {
        // Trigger error state
...
@Test
fun `pauseDrive should handle repository failure gracefully`() =
    runTest {
        // Start driving first
        coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
        viewModel.startDrive()
...
@Test
fun `resumeDrive should handle repository failure gracefully`() =
    runTest {
...
@Test
fun `stopDrive should be allowed from any state except emergency stop`() =
    runTest {
        coEvery { mockRepository.stopDrive() } returns Result.success(Unit)
...
@Test
fun `speed boundary validation should handle maximum allowed speed`() =
    runTest {
...
@Test
fun `speed boundary validation should reject speed above maximum`() =
    runTest {
...
@Test
fun `startDrive should handle SecurityException gracefully`() =
    runTest {
...
@Test
fun `viewModel should handle null exception messages gracefully`() =
    runTest {
...
@Test
fun `direction change should validate state transitions properly`() =
    runTest {
        // Start driving
        coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
        coEvery { mockRepository.changeDirection(any()) } returns Result.success(Unit)
...
@Test
fun `emergency stop from paused state should work correctly`() =
    runTest {
        // Start and pause
        coEvery { mockRepository.startDrive(any()) } returns Result.success(Unit)
        coEvery { mockRepository.pauseDrive() } returns Result.success(Unit)
...
@Test
fun `network connectivity check should handle NetworkManager exceptions`() =
    runTest {
...
@Test
fun `loading state should be properly managed during failed operations`() =
    runTest {
        coEvery { mockRepository.startDrive(any()) } coAnswers {
            kotlinx.coroutines.delay(100)
...
@Test
fun `multiple observers should all receive state updates`() =
    runTest {
...
@Test
fun `reset should work from any state`() =
    runTest {
        val states =
            listOf(
                {
...
@Test
fun `coroutine cancellation should be handled gracefully`() =
    runTest {
        coEvery { mockRepository.startDrive(any()) } coAnswers {
