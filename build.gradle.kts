...
fun collectModuleReports(): List<ModuleReport> {
    return subprojects.map { subproject ->
        ModuleReport(
            ...
            type = when {
                subproject.plugins.hasPlugin("com.android.build.tools") -> "agp-plugin"
...
// Define some useful version strings for status output (use version catalog when available)
val libsCatalog = extensions.findByType(VersionCatalogsExtension::class.java)?.named("libs")
val kotlinVersion = libsCatalog?.findVersion("kotlin")?.get()?.toString() ?: "unspecified"
val agpVersion = libsCatalog?.findVersion("agp")?.get()?.toString() ?: "unspecified"
val hiltVersion = libsCatalog?.findVersion("hilt")?.get()?.toString() ?: "unspecified"
